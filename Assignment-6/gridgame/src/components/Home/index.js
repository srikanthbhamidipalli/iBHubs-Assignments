...
class Home extends React.Component {
  ...
  handleChange = () => {
    this.state.className == "page-header-light"
      ? this.setState({ className: "page-header-dark" })
      : this.setState({ className: "page-header-light" });
  };
    ...
    for (let i = 0; i < this.state.grid_size * this.state.grid_size; i++) {
      ...
      if (this.state.randomNumbers.includes(i)) {
        each = (
          <GridTile
            value={i}
            key={this.state.level + "-" + i + this.keyText}
            level={this.state.level}
            correct={this.state.randomNumbers}
            className="initial-grid-tile"
            ms={this.state.grid_size * 1000}
